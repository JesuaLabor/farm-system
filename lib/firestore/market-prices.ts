...
export async function getMarketPrices(): Promise<MarketPrice[]> {
  const q = query(collection(getDb(), PRICE_COL), orderBy("cropName", "asc"));
...
export async function upsertMarketPrice(
  ...
  const ref = await addDoc(collection(getDb(), PRICE_COL), { ...data });
  return ref.id;
}
...
export async function updateMarketPrice(
  ...
  await updateDoc(doc(getDb(), PRICE_COL, id), data);
}
...
export async function getPriceTrend(cropName: string): Promise<PriceTrendEntry[]> {
  ...
  const snap = await getDoc(doc(getDb(), TREND_COL, trendDocId));
